 ...
 * @returns {{ Promise }}
 */
BizObject.prototype.list = function(){
    var self = this;
    ...
    // Recupera template
        ...
        // Template
        this.engine.render(this.path.asString + '/' + ret.template, this.params.state, 'modulos')
            .then((templ) => {
    ...
    // Recupera provider
        ...
        return (this.params['provider'] && this.params['provider']['id']
...
BizObject.prototype.select = function(provider, params, from){
    var dts;
    return new Promise((resolve, reject) => {

        // Conexão
        this.engine.getConnection(this.path.asArray)
            .then((d) => {
                dts = d;

                // Pega o provider
                this.getProvider(provider, from)
                    .then((provider) => {

                        // Customiza
                        extend(true, provider, params || {});

                        // Executa
                        dts.select(provider, this)
                            .then(
                                (data) => {
                                    resolve(data);
                                },
