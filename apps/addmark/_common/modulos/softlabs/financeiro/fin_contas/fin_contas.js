 ...
 */
tshark.modulos._add('softlabs.financeiro.fin_contas', {
    ...
    onAfterList: function(response, next){
        app.resetAppBar([
            {icon: 'payment icon', title: 'Contas Bancárias',  description: 'Gestão de Contas Financeiras'},
            {icon: 'add icon', title: 'Cadastrar Conta', description: 'Cadastrar nova conta', api: 'softlabs financeiro fin_contas create', active: 1}
        ]);
    },
    ...
    onBeforeEdit: function(el, settings){
        ...
        if (!$('.softlabs.financeiro.fin_contas-form').html()) {
            return true;
        }
